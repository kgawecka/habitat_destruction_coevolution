...
@everywhere include("antagonistic_v7a_master_functions.jl")
...
@everywhere function dynamicsD(n_p, n_a, n, dD, tmin, tmax, tol, ep, ep0, ea, cp, ca, M_inc, rep_no, networkName, plant_parameters, animal_parameters, alpha, sigma)
    ...
    @inbounds for k=1:(length(d)-1)
        ...
        n_p, x_p, n_a, x_a, z_p, z_a, n_patch, k = iterate_model_until_steady_state(tmax, x_p, x_a, z_p, z_a, n, n_p, n_a, k, x_state, M_inc, ep0, ep, cp, ea, alpha, ca, plant_parameters, animal_parameters, sigma, n_patch, tol, tmin)
  ...
  df_state = DataFrame(D = repeat(D, inner=n_patch),
                       ...
                       state = vec(x_state))
  CSV.write(string("../../Output_v7a/",networkName,"/antagonistic_out_",networkName,"_patch_state_r",rep_no,".csv"), df_state)
...
# julia -p 5 fragmentation_antagonistic_coevolution_v7a_modular.jl "A_HP_035"
