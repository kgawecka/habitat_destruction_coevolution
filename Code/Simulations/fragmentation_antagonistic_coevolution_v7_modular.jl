...
@everywhere include("antagonistic_coevolution_function.jl")
@everywhere include("construct_output_files_functions.jl")
@everywhere include("random_habitat_destruction_functions.jl")
@everywhere include("antagonistic_extinction_colonisation_functions.jl")
@everywhere include("antagonistic_v7_master_functions.jl")

# Define model function
@everywhere function dynamicsD(n_p, n_a, n, dD, tmin, tmax, tol, ep, ep0, ea, cp, ca, M_inc, rep_no, networkName, plant_parameters, animal_parameters, alpha, sigma)
    ...
    @inbounds for k=1:(length(d)-1)
        ...
        n_p, x_p, n_a, x_a, z_p, z_a, n_patch, df_ab_traits_p, df_ab_traits_a, k = iterate_model_until_steady_state(tmax, x_p, x_a, z_p, z_a, n, n_p, n_a, k, x_state, M_inc, ep0, ep, cp, ea, alpha, ca, plant_parameters, animal_parameters, sigma, df_ab_traits_p, df_ab_traits_a, n_patch, tol, tmin, t_sim)
  ...
  df_state = DataFrame(D = repeat(D, inner=n_patch),
                       ...
                       state = vec(x_state))
  CSV.write(string("../../Output_v7/",networkName,"/antagonistic_out_",networkName,"_patch_state_r",rep_no,".csv"), df_state)
...
@everywhere function run_parallel(replicate, networkName)
  ...
  ep0 = readdlm(string("../../Data/",networkName,"/ep0.csv"), ',', Float64)
  ...
  dynamicsD(n_p, n_a, n, dD, tmin, tmax, tol, ep, ep0, ea, cp, ca, M_inc, rep_no, networkName, plant_parameters, animal_parameters, alpha, sigma)
end
...
# julia -p 5 fragmentation_antagonistic_coevolution_v7_modular.jl "A_HP_035"
