...
function store_and_write_results(n_p, x_p, n_a, x_a, z_p, z_a, t_sim, n_patch, networkName, rep_no, D, k)
	...
	# output results
    CSV.write(string("../../Output_v7a/",networkName,"/antagonistic_out_",networkName,"_p_plants_r",rep_no,"_D",floor(Int,D[k]*100),".csv"), df_p_plants)
    CSV.write(string("../../Output_v7a/",networkName,"/antagonistic_out_",networkName,"_p_animals_r",rep_no,"_D",floor(Int,D[k]*100),".csv"), df_p_animals)
...
function iterate_model_until_steady_state(tmax, x_p, x_a, z_p, z_a, n, n_p, n_a, k, x_state, M_inc, ep0, ep, cp, ea, alpha, ca, plant_parameters, animal_parameters, sigma, n_patch, tol, tmin)
