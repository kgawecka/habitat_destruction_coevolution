...
function get_trait_matching_partners_patch(z_p_old, z_a_old, i, j, h, a_part_sp, alpha, sigma)

	# get trait value of plant
	z_p_s = z_p_old[i,j,h]

	# get traits of animal partners
	z_a_part = z_a_old[i,j,a_part_sp]

	# compute trait differences
	z_dif = z_p_s .- z_a_part

	# compute trait matching
	p_match = exp.(-alpha.*((z_dif).^2))

	# no increase in extinction prob if d_dif > sigma
	p_match[findall(abs.(z_dif) .> sigma)] .= 0
...
function compute_resource_extinction_prob(ep0, h, a_part_sp, p_match, ep)

	# "no extinction" probability
	pe_p = (1-ep0[h])
	...
	for m=a_part_sp
    	...
    	if p_match[j_count] === 0

			prod = 1

    	else

    		prod = (1 - ep[h,m] / j_count) * (1 - p_match[j_count] * ep[h,m])
...
function resource_extinctions(h, M_inc, x_a_old, i, j, z_p_old, z_a_old, ep0, ep, x_p, z_p, alpha, sigma)

	# get animal partners present in current patch
	a_part_sp = get_animal_partners_in_current_patch(i, j, h, M_inc, x_a_old)

	# get trait matching between partners in current patch
	p_match = get_trait_matching_partners_patch(z_p_old, z_a_old, i, j, h, a_part_sp, alpha, sigma)

	# compute extinction probabilities
	pe_p = compute_resource_extinction_prob(ep0, h, a_part_sp, p_match, ep)
...
function resource_colonisations(neigh, x_p_old, z_p_old, cp, i, j, h, x_p, z_p)
...
function resource_extinctions_and_colonisations(n_p, x_p_old, i, j, M_inc, x_a_old, z_p_old, z_a_old, ep0, ep, x_p, z_p, neigh, cp, alpha, sigma)
    ...
    for h=1:n_p
        ...
        if x_p_old[i,j,h] === 1
    	x_p, z_p = resource_extinctions(h, M_inc, x_a_old, i, j, z_p_old, z_a_old, ep0, ep, x_p, z_p, alpha, sigma)
        continue
        end

        # if species absent -> colonisations
        if x_p_old[i,j,h] == 0
        x_p, z_p = resource_colonisations(neigh, x_p_old, z_p_old, cp, i, j, h, x_p, z_p)
        end
