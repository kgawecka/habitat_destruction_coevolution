# GENERATE SPECIES PARAMETERS - ANTAGONISM
...
ep0 = rand(Uniform(0.14, 0.16), n_p)
ep = rand(Uniform(0.14, 0.16), n_p, n_a) .* M_inc
ea = rand(Uniform(0.14, 0.16), n_a)
cp = rand(Uniform(0.14, 0.16), n_p)
ca = rand(Uniform(0.14, 0.16), n_p, n_a) .* M_inc

writedlm(string("ep0.csv"), ep0, ",")
